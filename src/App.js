...
class App extends Component {
  ...
  render() {
    ...
    drawerItems.push(this.createDrawerButton("undoButton", UndoIcon, "Undo", () => {
      this.game.undo();
      return this.setState({
        gameState: this.game.state
      });
    }
    , null, !this.game.canUndo())
    );
    drawerItems.push(el(Divider, { key: "undoDivider" }));

    drawerItems.push(this.createDrawerButton("helpMenu", BookIcon, `Rule Help: ${this.game.modes[this.game.mode].name}`, () => {
      return this.setState({
        helpOpen: true,
        drawerOpen: false
      });
    ...
    drawerItems.push(this.createDrawerButton("hardMenu", SickIcon, "Hard Mode", () => {
    ...
    drawerItems.push(this.createDrawerButton("playAgainMenu", ReplayIcon, `Play Again: ${this.game.modes[this.game.mode].name}`, () => {
      this.game.newGame();
      return this.setState({
        drawerOpen: false,
        gameState: this.game.state
    ...
    for (var modeName in this.game.modes) {
      ...
      ((drawerItems, modeName, mode) => {
        return drawerItems.push(this.createDrawerButton(`newGame${modeName}`, GamesIcon, `New Game: ${mode.name}`, () => {
    ...
    if (fullscreen.available()) {
      ...
      drawerItems.push(this.createDrawerButton("fullscreen", FullscreenIcon, "Toggle Fullscreen", () => {
        fullscreen.toggle();
        return this.setState({
          drawerOpen: false
        });
    ...
    const menuButton = el(IconButton, {
      key: 'menuButton',
      size: 'large',
      style: {
        position: 'fixed',
    ...
    const winToastAction = el(React.Fragment, {
    ...
    const loseToastAction = el(React.Fragment, {
    ...
    const gameText = el('div', {
      ...
      el('div', { key: "gameAndDiff" }, `${this.game.modes[this.game.mode].name}${this.game.state.hard ? ' (Hard)' : ''}`)
    ]);
