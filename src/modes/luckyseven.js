...
`,
    ...
    placeSquad(squad, xPos, yPos) {
        return (() => {
            const result = []
            for (
                let squadIndex = 0, end = squad.length, asc = 0 <= end;
                asc ? squadIndex < end : squadIndex > end;
                asc ? squadIndex++ : squadIndex--
            ) {
            ...
            (() => {
                ...
                for (let rowIndex = 0; rowIndex < 4; rowIndex++) {
                    ...
                    if (sq != null && sq.tx !== -1 && sq.ty !== -1) {
                        var { bad } = this.state.grid[sq.tx][sq.ty]
                        if (bad != null) {
    ...
    luckyCounts() {
        ...
        for (let colIndex = 0; colIndex < 6; colIndex++) {
            for (var rowIndex = 0; rowIndex < 4; rowIndex++) {
                ...
                var { bad } = this.state.grid[colIndex][rowIndex]
                if (bad != null) {
                    counts.bads += 1
                }
            }
                            ...
                            (() => {
                                ...
                                for (var adj of Array.from(adjs)) {
                                    if (this.state.grid[adj.x][adj.y].bad != null) {
                                        this.luckyLog(
                                            `Killed: ${this.luckyName(this.state.grid[colIndex][rowIndex].squad.raw)}`,
                                            "#fcc"
                                        )
