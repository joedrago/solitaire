...
class SolitaireGame {
  ...
  click(type, outerIndex, innerIndex, isRightClick, isMouseUp) {
    if (outerIndex == null) { outerIndex = 0; }
    if (innerIndex == null) { innerIndex = 0; }
    if (isRightClick == null) { isRightClick = false; }
    if (isMouseUp == null) { isMouseUp = false; }
  ...
  sendHome(type, outerIndex, innerIndex) {
    ...
    switch (type) {
      case 'pile':
        ...
        break;
      case 'reserve':
        if ((this.state.reserve != null) && (this.state.reserve.cols.length > outerIndex) && (this.state.reserve.cols[outerIndex].length > 0)) {
  ...
  select(type, outerIndex, innerIndex) {
    if (outerIndex == null) { outerIndex = 0; }
    if (innerIndex == null) { innerIndex = 0; }
    if ((this.state.selection.type === type) && (this.state.selection.outerIndex === outerIndex) && (this.state.selection.innerIndex === innerIndex)) {
      // Toggle
      type = 'none';
      outerIndex = 0;
      innerIndex = 0;
