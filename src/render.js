...
const CARD_URLS = [ card0, card1, card2, card3, card4, card5, card6, card7, card8, card9,
...
export var card = function(cardInfo, renderInfo, listenerInfo) {
  ...
  if (isSelected) {
    x -= 1;
    y -= 1;
    if ((Math.abs(renderInfo.view.state.selectOffsetX) > renderInfo.dragSnapPixels) || (Math.abs(renderInfo.view.state.selectOffsetY) > renderInfo.dragSnapPixels)) {
  ...
  if (renderInfo.view.props.useTouch) {
    ...
    imageProps.onTouchEnd = function(e) {
      ...
      if ((dataType != null) && (dataOuterIndex != null) && (dataInnerIndex != null)) {
        return onClick(dataType, parseInt(dataOuterIndex), parseInt(dataInnerIndex), e.changedTouches[0].pageX, e.changedTouches[0].pageY, false, true);
      } else {
