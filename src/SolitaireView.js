...
class SolitaireView extends Component {
  ...
  tooCloseToDrag() {
    const dragDistanceSquared = ((this.state.selectMaxOffsetX * this.state.selectMaxOffsetX) + (this.state.selectMaxOffsetY * this.state.selectMaxOffsetY));
  ...
  cardClick(type, outerIndex, innerIndex, x, y, isRightClick, isMouseUp) {
    ...
    if (!isMouseUp) {
      ({
        type
      } = this.props.app.state.gameState.selection);
      if (type !== 'none') {
  ...
  render() {
    let cardInfo, colIndex, drawCard, drawOffsetL, drawOffsetT, grid, isSelected, raw, renderOffsetL, renderScale, rowIndex, wideGrid;
    const {
      gameState
    } = this.props;
    ...
    if ((workBottom < 4.1) && ((gameState.draw.pos === 'middle') || ((gameState.reserve != null) && (gameState.reserve.pos === 'middle')))) {
    ...
    if ((gameState.draw.pos === 'top') || (gameState.draw.pos === 'middle')) {
      ...
      for (let pileIndex = startPileIndex, end = gameState.pile.cards.length, asc = startPileIndex <= end; asc ? pileIndex < end : pileIndex > end; asc ? pileIndex++ : pileIndex--) {
        ...
        ((pile, pileIndex, isSelected) => {
          cardInfo = {
            key: `pile${pileIndex}`,
            raw: pile,
            x: drawOffsetL + renderOffsetL + ((1 + CENTER_CARD_MARGIN + ((pileIndex - startPileIndex) * PILE_CARD_OVERLAP)) * this.renderScalePixels),
            y: drawOffsetT + renderOffsetT,
            selected: isSelected,
            type: 'pile',
    ...
    for (let workColumnIndex = 0; workColumnIndex < gameState.work.length; workColumnIndex++) {
      ...
      for (var workIndex = 0; workIndex < workColumn.length; workIndex++) {
        ...
        isSelected = false;
        if ((gameState.selection.type === 'work') && (workColumnIndex === gameState.selection.outerIndex) && (workIndex >= gameState.selection.innerIndex)) {
          isSelected = true;
    ...
    for (let foundationIndex = 0; foundationIndex < gameState.foundations.length; foundationIndex++) {
      ...
      if ((this.state.sent != null) && (this.state.sent.foundationIndex === foundationIndex) && (this.state.sent.prevFoundationRaw >= 0)) {
        ((foundation, foundationIndex) => {
          cardInfo = {
    ...
    if (gameState.reserve != null) {
      ...
      for (colIndex = 0; colIndex < gameState.reserve.cols.length; colIndex++) {
        ...
        if (col.length > 0) {
          ...
          isSelected = false;
          if ((gameState.selection.type === 'reserve') && (colIndex === gameState.selection.outerIndex) && (reserveIndex >= gameState.selection.innerIndex)) {
            isSelected = true;
          }
          cardInfo = {
    ...
    if ((gameState.timerStart != null) && (gameState.timerColor != null)) {
      ...
      this.renderedCards.push(el('div', {
        key: 'timer',
        style: {
          position: 'fixed',
          textAlign: 'center',
    ...
    if (this.autowinInterval || this.props.canAutoWin) {
      this.renderedCards.push(el(IconButton, {
        key: 'autowinButton',
        size: 'large',
        style: {
          position: 'fixed',
    ...
    if (gameState.grid != null) {
      ...
      if ((gameState.log != null) && (gameState.log.length > 0)) {
        ...
        if (wideGrid) {
          ...
          logT = renderOffsetT + (this.renderScalePixels * ((render.CARD_HEIGHT - render.CARD_WIDTH) / render.CARD_HEIGHT)) + logTextSize;
        } else {
          logL = renderOffsetL;
      ...
      if ((this.props.app.tweens != null) && (this.props.app.tweens.length > 0) && (this.props.app.tweens[0].grid != null)) {
        ...
        ({
          grid
        } = this.props.app.tweens[0]);
      }
      for (colIndex = 0, end1 = grid.length, asc1 = 0 <= end1; asc1 ? colIndex < end1 : colIndex > end1; asc1 ? colIndex++ : colIndex--) {
        cardInfo = {
      ...
      for (rowIndex = 0, end2 = grid[0].length, asc2 = 0 <= end2; asc2 ? rowIndex < end2 : rowIndex > end2; asc2 ? rowIndex++ : rowIndex--) {
        raw = cardutils.ROAD;
      ...
      for (colIndex = 0, end3 = grid.length, asc3 = 0 <= end3; asc3 ? colIndex < end3 : colIndex > end3; asc3 ? colIndex++ : colIndex--) {
        var asc4, end4;
        for (rowIndex = 0, end4 = grid[0].length, asc4 = 0 <= end4; asc4 ? rowIndex < end4 : rowIndex > end4; asc4 ? rowIndex++ : rowIndex--) {
          ...
          isSelected = false;
          if ((gameState.selection.type === 'grid') && (colIndex === gameState.selection.outerIndex) && (rowIndex === gameState.selection.innerIndex)) {
            isSelected = true;
          }
          raw = cardutils.GRID;
          cardInfo = {
          ...
          if ((sq != null) && !isSelected && (sq.tx >= 0)) {
            ...
            if (arrowAngle != null) {
              this.renderedCards.push(el('img', {
      ...
      if (gameState.phase != null) {
        ...
        this.renderedCards.push(el('div', {
          ...
          style: {
            ...
            top: (renderOffsetT + (this.renderScalePixels * (1 + CENTER_CARD_MARGIN))) - (renderScale * render.CARD_WIDTH),
            fontFamily: 'monospace',
